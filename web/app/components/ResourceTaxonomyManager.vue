...
async function save() {
  ...
  try {
    ...
    resetSave()
    saveError.value = err?.data?.message || '保存失败，请检查名称与 slug 后重试'
  }
}
...
function armDelete() { deleteArmed.value = true }
function cancelDelete() { deleteArmed.value = false }
</script>
...
<template>
  <div class="space-y-5">
    ...
    <template v-else>
      ...
      <ManageEmpty v-if="!rows.length" :icon="kind === 'category' ? 'i-tabler-folder-off' : 'i-tabler-hash-off'" :text="q ? `没有匹配的${title}` : `还没有${title}`" />

      <section v-else class="overflow-hidden rounded-xl border border-default bg-default">
        <div class="divide-y divide-default">
          ...
          >
            <span class="flex min-w-0 items-center gap-3" :style="kind === 'category' ? { paddingLeft: `${Math.min(row.depth, 4) * 22}px` } : undefined">
              <UIcon v-if="kind === 'category' && row.depth > 0" name="i-tabler-corner-down-right" class="size-4 shrink-0 text-dimmed" />
            ...
            <span class="col-start-1 pl-13 text-xs text-muted lg:col-start-auto lg:pl-0 lg:text-right">
              <span class="font-semibold text-highlighted">{{ row.tax.count || 0 }}</span> 个资源
            </span>
      ...
      <template #body>
        <div class="space-y-5">
          ...
          <template v-if="current">
            <USeparator />
            <section aria-labelledby="resource-taxonomy-management" class="space-y-4">
              <div>
                <h3 id="resource-taxonomy-management" class="text-sm font-medium text-highlighted">管理{{ title }}</h3>
                <p class="mt-1 text-xs text-muted">合并会迁移关联资源；删除有关联或子分类时会被后端拒绝。</p>
              </div>
              ...
              <div class="rounded-lg border border-error/30 bg-error/5 p-3">
                <div v-if="!deleteArmed" class="flex items-start justify-between gap-3">
                  <div>
                    <p class="text-sm font-medium text-highlighted">删除{{ title }}</p>
                ...
                <div v-else>
                  ...
                  <div class="mt-3 flex justify-end gap-2">
                    ...
                    <UButton label="确认删除" color="error" size="sm" :loading="operationBusy === 'delete'" @click="deleteCurrent" />
                  </div>
                </div>
              </div>
            </section>
