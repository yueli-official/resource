...
const tags = computed(() => [...(tagsData.value?.items ?? [])].sort((a, b) => b.count - a.count).slice(0, 18))
...
const featuredResources = computed(() => {
  ...
  const configured = (homeSettings.value.featuredResourceIds || []).map(id => byId.get(id)).filter(Boolean) as ResourceView[]
  if (configured.length) return configured.slice(0, 3)
  return [...resources.value].sort((a, b) => b.downloadCount - a.downloadCount).slice(0, 3)
})
const lead = computed(() => featuredResources.value[0] || resources.value[0])
const popular = computed(() => [...resources.value].sort((a, b) => b.downloadCount - a.downloadCount).slice(0, 5))
    ...
    <header class="border-b border-default pb-7">
      <div class="max-w-3xl">
        <div class="flex items-center gap-2 text-primary">
          ...
          <span class="text-xs font-semibold uppercase tracking-[0.16em]">Resource Library</span>
        </div>
        <h1 class="font-display mt-3 text-4xl font-bold leading-[1.05] tracking-tight text-highlighted sm:text-5xl">{{ homeSettings.heroTitle }}</h1>
        <p class="mt-4 max-w-2xl text-base leading-7 text-muted">{{ homeSettings.heroSubtitle }}</p>
        <form class="mt-6 flex max-w-2xl gap-2" @submit.prevent="goSearch">
          <UInput v-model="searchQ" icon="i-tabler-search" placeholder="搜索软件、素材、脚本…" size="lg" class="min-w-0 flex-1" />
          <UButton type="submit" icon="i-tabler-search" label="搜索" size="lg" :disabled="!searchQ.trim()" />
        </form>
      </div>
    </header>

    <section v-if="lead" class="grid gap-6 md:h-[450px] md:grid-cols-[minmax(0,1.7fr)_minmax(0,1fr)] md:gap-8">
      <NuxtLink
      ...
      >
        <div class="relative aspect-[16/9] overflow-hidden bg-elevated md:aspect-auto md:min-h-0 md:flex-1">
          <img v-if="lead.coverUrl" :src="lead.coverUrl" :alt="lead.title" class="size-full object-cover transition duration-700 group-hover:scale-[1.03]">
          <div v-else class="grid size-full place-items-center bg-gradient-to-br from-primary/15 to-elevated text-primary/40">
            <UIcon name="i-tabler-package" class="size-20 md:size-24" />
          </div>
          <span class="absolute left-4 top-4 inline-flex items-center gap-1 rounded-full bg-default/90 px-3 py-1 text-xs font-semibold text-primary shadow-sm backdrop-blur">
            <UIcon name="i-tabler-sparkles" class="size-3.5" />精选资源
          </span>
        </div>
        <div class="flex flex-col p-5 sm:p-6 md:flex-none">
          <div class="flex flex-wrap items-center gap-2 text-xs text-muted">
            <span class="flex items-center gap-1"><UIcon name="i-tabler-download" class="size-3.5" />{{ lead.downloadCount }} 次下载</span>
            <span>/{{ lead.slug }}</span>
          </div>
          <h2 class="font-display mt-3 line-clamp-1 text-xl font-bold leading-tight text-highlighted transition group-hover:text-primary sm:text-2xl">{{ lead.title }}</h2>
          <p class="mt-2 line-clamp-2 min-h-[2lh] text-sm leading-6 text-muted">{{ lead.summary || '未填写摘要' }}</p>
        </div>
      </NuxtLink>
      ...
      <div v-if="recommended.length" class="flex flex-col">
        ...
        <ul class="flex flex-col divide-y divide-default">
          <li v-for="(item, i) in recommended" :key="item.id">
            <NuxtLink :to="`/resources/${item.id}`" class="group flex items-center gap-4 py-4">
              <span class="font-display w-7 shrink-0 text-lg font-bold tabular-nums text-primary/30">{{ String(i + 1).padStart(2, '0') }}</span>
              <div class="min-w-0">
                <h3 class="font-display line-clamp-2 font-semibold leading-snug text-highlighted transition group-hover:text-primary">{{ item.title }}</h3>
                <div class="mt-1.5 flex items-center gap-3 text-xs text-muted">
                  <span class="flex items-center gap-1"><UIcon name="i-tabler-download" class="size-3.5" />{{ item.downloadCount }} 次下载</span>
                  <span>/{{ item.slug }}</span>
                </div>
              </div>
            </NuxtLink>
    ...
    <section class="grid gap-10 lg:grid-cols-[minmax(0,1fr)_280px]">
      <div class="min-w-0">
        <div class="mb-5 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <h2 class="font-display text-xl font-semibold text-highlighted">最新资源</h2>
        ...
        <div v-else-if="!resources.length" class="rounded-lg border border-dashed border-default py-20 text-center text-muted">
          <div class="mx-auto grid size-14 place-items-center rounded-lg bg-primary/10 text-primary"><UIcon name="i-tabler-package-off" class="size-7" /></div>
          <p class="mt-3 text-sm">还没有资源</p>
        </div>
        <div v-else class="grid gap-4 [grid-template-columns:repeat(auto-fill,minmax(min(17rem,100%),1fr))]">
      ...
      <aside class="space-y-8 lg:sticky lg:top-20 lg:self-start">
        <div v-if="visibleCategories.length">
          <h3 class="mb-3 flex items-center gap-2 font-display font-semibold text-highlighted">
        ...
        <div v-if="tags.length">
          ...
          <div class="flex flex-wrap gap-1.5">
            <NuxtLink v-for="tag in tags" :key="tag.id" :to="`/tags/${tag.slug}`" class="rounded-full bg-elevated px-2.5 py-1 text-xs text-muted transition hover:text-primary">#{{ tag.name }}</NuxtLink>
          </div>
        </div>
      </aside>
    </section>
