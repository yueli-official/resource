...
test('builds the canonical named Asset media URL', () => {
  assert.equal(
    ...
    '/media/31Pj0mXv7cfR5fdZIUvra?format=webp&preset=display&v=1'
  )
})
...
test('accepts a public media origin without leaking transform parameters', () => {
  ...
  assert.equal(url, 'https://img.yueli.dev/media/31Pj0mXv7cfR5fdZIUvra?format=webp&preset=card&v=1')
