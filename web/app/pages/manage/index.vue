...
function changeCollectionControl(id: string, value: CollectionControlValue) {
  if (id === 'sort' && sorts.includes(value as ResourceSort)) sort.value = value as ResourceSort
  ...
  <div class="space-y-6">
    ...
    <div class="space-y-6" :inert="batchBusy" :aria-busy="batchBusy">
      ...
      <UAlert
        ...
        :title="batchResult.interrupted ? '批量请求中断' : `已处理 ${batchResult.changed} 个资源`"
        :description="batchResult.interrupted ? batchResult.message : (batchResult.failures.length ? `${batchResult.failures.length} 个资源仍未完成。` : '批量操作已完成。')"
        :actions="batchResult.failures[0] ? [{ label: '查看首个失败项', to: `/manage/${batchResult.failures[0].id}`, color: 'warning', variant: 'link' }] : undefined"
        close
      ...
      >
        ...
        <template #columns>
          <div class="grid grid-cols-[minmax(0,1fr)_11rem_5rem] items-center gap-3">
            <span>名称、类型与标签</span>
        ...
        <template #bulk-actions>
          <USelect v-model="batchAction" :items="batchItems" value-key="value" placeholder="批量操作" size="xs" class="w-28" :disabled="batchBusy" aria-label="批量操作" />
        ...
        <template #item="{ item: resource }">
          <div v-if="viewMode === 'list'" class="grid min-w-0 grid-cols-[minmax(0,1fr)_11rem_5rem] items-center gap-3">
            <div class="flex min-w-0 items-center gap-3">
              <div class="size-16 shrink-0 overflow-hidden rounded-md border border-default bg-elevated">
                <img v-if="resource.coverUrl" :src="resource.coverUrl" :alt="resource.title" class="size-full object-cover">
                <div v-else class="grid size-full place-items-center bg-primary/10 text-primary"><UIcon name="i-tabler-package" class="size-6" /></div>
              </div>
              <div class="min-w-0">
                <NuxtLink :to="`/manage/${resource.id}`" class="block truncate text-sm font-semibold text-highlighted hover:text-primary">{{ resource.title }}</NuxtLink>
                ...
                <p v-if="resource.issueCount" class="mt-1 inline-flex items-center gap-1 text-xs text-warning"><UIcon name="i-tabler-alert-circle" class="size-3.5" />待完善</p>
              </div>
            </div>
            <div class="min-w-0 text-right text-xs">
              ...
              <ClientOnly><p class="mt-1 truncate text-dimmed">{{ resourceMeta(resource) }}</p><template #fallback><p class="mt-1 text-dimmed">…</p></template></ClientOnly>
            </div>
            <div class="flex justify-end gap-1">
              <UTooltip text="快速编辑"><UButton icon="i-tabler-pencil" color="neutral" variant="ghost" size="xs" square :aria-label="`快速编辑：${resource.title}`" @click="openQuickEdit(resource)" /></UTooltip>
              <UTooltip text="完整编辑"><UButton :to="`/manage/${resource.id}`" icon="i-tabler-file-pencil" color="neutral" variant="ghost" size="xs" square :aria-label="`完整编辑：${resource.title}`" /></UTooltip>
            </div>
          </div>

          <div v-else class="group -m-4 overflow-hidden rounded-lg">
            <NuxtLink :to="`/manage/${resource.id}`" class="block focus:outline-none" :aria-label="`查看资源：${resource.title}`">
              <div class="relative h-[clamp(7.5rem,13vw,9.5rem)] overflow-hidden border-b border-default bg-elevated">
                ...
                <div v-else class="grid size-full place-items-center bg-primary/10 text-primary"><UIcon name="i-tabler-package" class="size-8" /></div>
              </div>
              <div class="min-w-0 p-3">
                ...
                <div class="mt-3 flex items-end justify-between gap-2 border-t border-default pt-2.5">
                  <div><p class="text-xs text-muted">下载</p><p class="text-sm font-semibold text-highlighted">{{ resource.downloadCount }}</p></div>
                  <ClientOnly><p class="text-xs text-dimmed">{{ resource.updatedAt ? abs(resource.updatedAt) : '-' }}</p><template #fallback><span class="text-xs text-dimmed">…</span></template></ClientOnly>
                </div>
              </div>
            </NuxtLink>
            <UTooltip text="快速编辑"><UButton icon="i-tabler-pencil" color="neutral" variant="solid" size="xs" square class="absolute right-4 top-4 z-10" :aria-label="`快速编辑：${resource.title}`" @click.stop="openQuickEdit(resource)" /></UTooltip>
          </div>
        </template>
      </CollectionPanel>
    </div>
