...
export default defineNuxtConfig({
  ...
  runtimeConfig: {
    ...
    identityBase: process.env.NUXT_IDENTITY_BASE || process.env.NUXT_PUBLIC_OIDC_ISSUER || 'http://localhost:8081',
    assetBase: process.env.NUXT_ASSET_BASE || 'http://127.0.0.1:8082',
    // OIDC BFF (layer) config — override in prod with NUXT_* env.
    sealSecret: process.env.NUXT_SEAL_SECRET || 'dev-resource-seal-secret-change-me-0123456789ab',
    ...
    public: {
      ...
      oidcClientId: process.env.NUXT_PUBLIC_OIDC_CLIENT_ID || 'resource-ae-web',
      oidcRedirectUri: process.env.NUXT_PUBLIC_OIDC_REDIRECT_URI || 'http://localhost:3001/auth/callback',
      ...
      siteSlug: process.env.NUXT_PUBLIC_SITE_SLUG || 'resource-ae',
      siteBrand,
      siteDomain: process.env.NUXT_PUBLIC_SITE_DOMAIN || 'resource-ae.localhost',
